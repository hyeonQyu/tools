...
export type ExpirationDateSwipeDirection = 'prev' | 'next';

interface UseExpirationDateSwipeParams {
  onNavigate: (direction: ExpirationDateSwipeDirection) => void;
}

export const useExpirationDateSwipe = ({ onNavigate }: UseExpirationDateSwipeParams) => {
  ...
  const [peekPanel, setPeekPanel] = useState<ExpirationDateSwipeDirection | null>(null);
  ...
  const openPeekPanel = (direction: ExpirationDateSwipeDirection) => {
    peekOffsetRef.current = direction === 'prev' ? -getWidth() : getWidth();
    setPeekPanel(direction);
  };

  const clearPeekPanel = () => setPeekPanel(null);
  ...
  const commitToAdjacentMonth = (direction: ExpirationDateSwipeDirection, duration: number) => {
    const targetX = direction === 'next' ? -getWidth() : getWidth();
    animate(x, targetX, {
      duration,
      ease: 'easeOut',
      onComplete: () => {
  ...
  const navigateAdjacent = (direction: ExpirationDateSwipeDirection) => {
  ...
  const handleTouchMove = (e: TouchEvent<HTMLDivElement>) => {
    ...
    if (!swipeAxisRef.current) {
      ...
      if (swipeAxisRef.current === 'h') openPeekPanel(diffX > 0 ? 'prev' : 'next');
    }
    if (swipeAxisRef.current === 'h') x.set(diffX);
  };
  ...
  const handleTouchEnd = (e: TouchEvent<HTMLDivElement>) => {
    ...
    const shouldNavigate = Math.abs(diff) >= getWidth() * SWIPE_THRESHOLD_RATIO;
    if (!shouldNavigate) {
